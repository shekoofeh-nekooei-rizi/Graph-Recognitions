...
Moreover, to achieve linear time performance in LexBFS, vertex movement and partition update/refinement must be done in constant time.
...
This program correctly  identifies it as not chordal in under or around one minute (in my experiments).
Similarly, Feder_graph(10**7) is also very huge, with almost 30 million vertices and edges in total.
The program successfully recognizes it as a chordal graph in around  1 - 2  minutes."""

def Feder_graph(n):
    """
    This function generates a graph consists of a path of n-1 vertices and an additional vertex that is connected to all vertices of this path except the first and the last ones.
    """
    graph = {}
...
def test_Feder_graph(n):
    """
    Generates a Feder graph with n vertices, runs the is_chordal function on it,
...
def test_Mycielski(n):
    """
    Generates a Mycielski graph with n vertices, runs the is_chordal function on it,
    and prints a summary table including:
