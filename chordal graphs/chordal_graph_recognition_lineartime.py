...
This program determines if an input graph is chordal, in linear time with respect to the input graph size (number of vertices + number of edges).


I implement the classic algorithm introduced in "Algorithmic Aspects of Vertex Elimination on Graphs" by Rose, Tarjan, and Lueker.

The process starts by computing a **Lexicographical Breadth First Search (LexBFS)** ordering of vertices, using **partition refinement** technique, which I learned in "A Simple Linear Time Algorithm for Cographs Recognition" by Bretscher, Corneil, Habib, and Paul.

Next, it checks whether the reverse of this sequence forms a **perfect elimination ordering**  -- a property unique to chordal graphs.



To guarantee linear time performance in LexBFS,  vertex movement and partition update (refinement) must be done in constant time.

This is possible through operations on ` particular data structures `, using ` pointers ` and ` doubly linked lists `.

So, the data structure implementation isn't quite straightforward, but the program achieves the algorithm's expected linear time complexity.
...
It is fascinating to see that the program reaches the expected results fast, even for huge graphs with sizes over ten million.
"""

# This is a helper class, to represent each vertex of the input graph as a pointer object in a doubly linked lists.
...
def LexBFS(graph):
    """
    Computes a Lexicographical Breadth-First Search (LexBFS) ordering of the input graph in linear time.
    ...
    - Its neighbors are moved into a new partitions just before their current ones.
...
def is_chordal(graph):
    ...
    A graph is chordal if and only if the reverse of some Lexicographical Breadth First Search (LexBFS) order is a perfect elimination ordering (PEO).
...
g = {'1': ['2', '5'], '2': ['1', '3'], '3': ['2', '4'], '4': ['3', '5'], '5': ['4', '1'], '0': ['1', '2', '3', '4', '5'], }
...
It is worth noting that the level-16 Mycielski graph has a size of over 16 million (around 17 million edges and vertices), and this program correctly  recongnizes it as not chordal in under two minutes (around a minute in my experiments).
...
print("Level thirteen Mycielski: ", M13)
print("\nIs Mycielsky graph, level thirteen, chordal?\n")

is_chordal(Mycielski_13_valid_format)

M15 = mycielski_graph(15)
Mycielski_15_valid_format = {node: list(M15[node]) for node in M15.nodes()}

print("Level fifteen Mycielski: ", M15)
print("\nIs Mycielsky graph, level fifteen, chordal?\n")

is_chordal(Mycielski_15_valid_format)

M16 = mycielski_graph(16)
Mycielski_16_valid_format = {node: list(M16[node]) for node in M16.nodes()}

print("Level sixteen Mycielski: ", M16)
print("\nIs Mycielsky graph, level sixteen, chordal?\n")

is_chordal(Mycielski_16_valid_format)